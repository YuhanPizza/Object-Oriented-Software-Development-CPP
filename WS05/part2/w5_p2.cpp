...
//this is where this points to since it was called in library.setObserver(bookAddedObserver);
...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		 ...
		 */
		library.setObserver(bookAddedObserver); //void setObserver(void (*observer)(const Collection<T>&, const T&))
	...
	for (auto i = 3; i < argc; ++i)
	{
		...
		//         where ERROR_MESSAGE is extracted from the exception object.
		try {
			SpellChecker sp(argv[i]); //constructor call

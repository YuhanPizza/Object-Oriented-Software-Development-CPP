...
void loadData(const char* filename, sdds::College& theCollege)
{
	...
	while (file)
	{
		...
		catch (...) { //incase an unknown error occurs.
			std::cerr << "ERROR: An unknown exception occurred while extracting data.\n";
			return; //exit
		}
		//different Approach 
		/*sdds::Person* thePerson = nullptr;
...
int main(int argc, char** argv)
{
	...
	std::list<const sdds::Person*> persons;
	{
		...
		auto students = [](const sdds::Person* p) { return dynamic_cast<const sdds::Student*>(p) != nullptr; };
	...
	persons.clear();
	{
		...
		auto professors = [](const sdds::Person* p) { return dynamic_cast<const sdds::Professor*>(p) != nullptr; };
